...
-- =============================================
CREATE PROCEDURE [dbo].[selectDisplayMessages]
...
BEGIN
	...
	begin	--no active alarms, display default messages
		...
		begin
		    if (select columnname from AlarmDefaults where AlarmID=@i) not like '%reman%' and (select columnname from AlarmDefaults where AlarmID=@i) not like '%trimloss%'
				select @sqlstring = 'insert into #tempmessages select alarmid,Prefix + convert(varchar,convert(int,' + columnname + ')),0,1000,displaytime,blanktime,0,NULL,0 from currentstate,alarmgeneralsettings,alarmdefaults where active = 1 and alarmid=' + convert(varchar,@i) from alarmdefaults where columnname is not null and alarmid=@i
			else
				select @sqlstring = 'insert into #tempmessages select alarmid,Prefix + convert(varchar,convert(real,' + columnname + ')),0,1000,displaytime,blanktime,0,NULL,0 from currentstate,alarmgeneralsettings,alarmdefaults where active = 1 and alarmid=' + convert(varchar,@i) from alarmdefaults where columnname is not null and alarmid=@i
		...
		begin
			--Adaptive Display
				insert into #tempmessages select alarmid,convert(varchar,getdate(),100),0,1000 ,displaytime,blanktime,0,NULL,0
				from alarmdefaults,alarmgeneralsettings where category = 'Current Date & Time'
			
		end
		insert into #tempmessages select alarmid,prefix,0,1000 ,displaytime,blanktime,0,NULL,0
				from alarmdefaults,alarmgeneralsettings where active = 1 and category = 'User Message'
			insert into #tempmessages select alarmid, prefix + convert(varchar,abs(datediff(dd,getdate(),AccidentDate))),0,1000 ,displaytime,blanktime,0,NULL,0
				from alarmdefaults,alarmgeneralsettings where active = 1 and category = 'Safety'
				
		insert into #tempmessages select 2000+alarms.alarmid,displaytext,severity,priority,displaytime,blanktime,0,alarms.Data,datarequired from alarms,alarmsettings,alarmgeneralsettings 
		where stoptime is null /*and displaytext <> ''*/ and alarms.alarmid = alarmsettings.alarmid and active = 1 and Priority=1000
		insert into #tempmessages select 2000+alarmsprevious.alarmid,displaytext,severity,priority,displaytime,blanktime,0,alarmsprevious.Data,datarequired from alarmsprevious,alarmsettings,alarmgeneralsettings 
		where stoptime is null /*and displaytext <> ''*/ and alarmsprevious.alarmid = alarmsettings.alarmid and active = 1 and Priority=1000
		
	end
	else
	begin	--show alarms
		insert into #tempmessages select alarms.alarmid,displaytext,severity,priority,displaytime,blanktime,0,alarms.Data,datarequired from alarms,alarmsettings,alarmgeneralsettings with(NOLOCK)
		...
		insert into #tempmessages select alarmsprevious.alarmid,displaytext,severity,priority,displaytime,blanktime,0,alarmsprevious.Data,datarequired from alarmsprevious,alarmsettings,alarmgeneralsettings with(NOLOCK)
	...
	begin
		...
		begin
			...
			begin
				...
				update #tempmessages set Data = (select Data from alarmsettings where AlarmID = @aid1) where AlarmID = @aid
				update alarms set Data = (select Data from alarmsettings where AlarmID = @aid1) where AlarmID = @aid and stoptime is null
