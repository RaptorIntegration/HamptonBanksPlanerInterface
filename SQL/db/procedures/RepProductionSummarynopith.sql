...
-- =============================================
create PROCEDURE [dbo].[RepProductionSummarynopith]
...
BEGIN
	...
	begin
		...
		and runs.RecipeID between @recipeidstart and @recipeidend) = 0
			select prodlabel='No Data',thicknominal=0,widthnominal=0,gradelabel='No Data',lengthnominal=0,lengthlabel='No Data',totalpieces=0,totalvolume=0
		else
			select prodlabel,thicknominal,widthnominal,gradelabel,lengthnominal,lengthlabel,totalpieces=sum(boardcount),totalvolume=sum(thicknominal*widthnominal*lengthnominal*boardcount/144)
	...
	begin
		...
		where sorted=1 and ( SortCode= 22) and shiftindex between @shiftstart and @shiftend
		and runs.runindex = productionboards.runindex
		and runs.RecipeID between @recipeidstart and @recipeidend)
