...
namespace WebSort
{
    public partial class DeviceModes : BasePage
    {
        private static User CurrentUser;

        protected void Page_Load(object sender, EventArgs e)
        {
            CurrentUser = Global.GetSecurity("Diagnostic Modes", User.Identity.Name);
        ...
        protected void TreeView1_TreeNodeCheckChanged(object sender, TreeNodeEventArgs e)
        {
            ...
            if (e.Node.Text.Contains("ncoder") && e.Node.Checked == true)
            {
                Timer3.Enabled = true;
        ...
        protected void ButtonDeleteItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                connection.Close();
                Session["DiagSelectedNode"] = null;
                Response.Redirect("devicemodes.aspx");
            }
            catch
            {
            ...
            if (Global.OnlineSetup)
            {
                ...
                try
                {
                    foreach (TreeNode n in childnodes)
                    {
                        if (Session["editing"].ToString() == "1")
                            return;
                        SqlCommand cmdtree = new SqlCommand("select diagnosticon from diagnostictests1 where deviceid=" + n.Value.ToString(), connection);
