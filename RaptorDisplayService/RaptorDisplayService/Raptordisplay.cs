...
namespace RaptorComm
{
    public static class RaptorCommService
    {
        ...
        public static void NewTryCatchException(int index, Exception ex)
        {
            Exceptions[index].index = index;
            Exceptions[index].exception = ex;
        }
        public struct Caught
        {
            public static string tryCatchPath = "C://RaptorWebSort/CaughtExceptions/";
            ...
            private void writeException()
            {
                try
                {
                    if (!Directory.Exists("C://RaptorWebSort/CaughtExceptions/"))
                        Directory.CreateDirectory("C://RaptorWebSort/CaughtExceptions/");

                    string path = tryCatchPath + DateTime.Now.ToFileTime() + "_" + numExceptions.ToString() + ".txt";

                    using (System.IO.StreamWriter writer = new System.IO.StreamWriter(path))
                    {
                        writer.WriteLine(DateTime.Now.ToLocalTime());
                        writer.WriteLine("At index: " + index.ToString());
        ...
        public static Caught[] Exceptions = new Caught[200];
        ...
        public static void ReadDisplaySettings()
        {
            ...
            if (reader.HasRows)
            {
                ...
                if (DisplayType == 1 || DisplayType == 3)  //Adaptive Display
                    if (connected[0] == false && DisplayCount > 0)
                        UpdateRaptorDisplayLog("Attempting to Connect to Display: " + szIPSelected[0]);
                ...
                else if (DisplayType == 3)  //EZMarquee Display
                {
                    try
                    {
                        string header;
                        ...
                        if (DisplayCount > 0)
                        {
                            serialPortDisplay.BaudRate = 9600;
                            serialPortDisplay.PortName = szPort;
                            serialPortDisplay.Parity = System.IO.Ports.Parity.None;
                            serialPortDisplay.Open();
                        }
                       ...
                       /* if (DisplayCount > 1)
                        {
                            ...
                            for (int i = 1; i < DisplayCount; i++)
                            {
                                ...
                                if (connected[i] == false)
                                {
                                    attempts[i] = attempts[i] + 1;
                                    //if (attempts[i] < 3)
                                    UpdateRaptorDisplayLog("Attempting to Connect to EZMarquee Display: " + szIPSelected[i]);
                                }
                            }
                        }*/
                       // else
                                ...
                                //if (i == 0 || (i > 0 && attempts[i] < 3))
                                {
                                    ...
                                    if (DisplayCount > 1)
                                    {
                                        ...
                                        //for (int ii = 1; ii < DisplayCount; ii++)
                                        {
                                            lastoctet = lastoctet + i;
                                            szIPSelected[i] = szIPSelected[0].Substring(0, szIPSelected[0].LastIndexOf(".") + 1) + lastoctet.ToString();
                                            UpdateRaptorDisplayLog("Successfully Connected to EZMarquee Display: " + szIPSelected[i]);
                                        }
                                    }
                                    else
                                        UpdateRaptorDisplayLog("Successfully Connected to EZMarquee Display: " + szIPSelected[0]);
                                }
                                //else
                                // UpdateRaptorDisplayLog("Giving up on " + szIPSelected[i]);
                            }
            ...
            while (true)
            {
                try
                {
                    ...
                    if (timecounter > 3000)
                    {
                        ...
                        cmd1.ExecuteNonQuery();
                        SqlCommand cmd1a = new SqlCommand("execute UpdateStatusDataWEBSort 480,1", connection);
                    ...
                    else
                    {
                        ...
                        SqlCommand cmd2a = new SqlCommand("execute UpdateStatusDataWEBSort 480,0", connection);
        ...
        public static void ReadMessages()
        {
            ...
            {                
                ...
                else if (DisplayType == 3)   //EZ Marquee
                {
                    try
                    {
                        string header;
                        ...
                        byte[] byData;
                        string message;
                        string message1, message2, messagetemp;
                        int lastindex;
                        ...
                        try
                        {
                            ...
                            while (reader.Read())
                            {
                                ...
                                {
                                    header = "";
                                    header = header + "^A000000^E1^C1";
                                    if (reader["severity"].ToString() == "0")
                                        header = header + "^dC2";  //Green
                                    else if (reader["severity"].ToString() == "1")
                                        header = header + "^dC3";  //Amber
                                    else if (reader["severity"].ToString() == "2" || reader["severity"].ToString() == "3")
                                        header = header + "^dC1";  //Red
                                    ...
                                    else //message length is greater than 20 characters, must split it into two lines
                                    {
                                        header = header + "^L0^K0";
                                        //find the best place to split it                                      

                                        messagetemp = message.Substring(0, 20);
                                        //UpdateRaptorDisplayLog(message);
                                        lastindex = messagetemp.LastIndexOf(" ");

                                        //UpdateRaptorDisplayLog(lastindex.ToString());
                                        message1 = message.Substring(0, lastindex);
                                        message2 = message.Substring(lastindex + 1);
                                        if (message2.Length > 20)
                                            message2 = message2.Substring(0, 20);
                                        header = header + message1 + "^N" + message2;
                                ...
                                else  //default message
                                {
                                    ...
                                    if (message.Contains((char)(13)))  //replace carriage return with ^N 
                                    ...
                                    byData = System.Text.Encoding.ASCII.GetBytes(objData.ToString());
                                        for (int i = 0; i < DisplayCount; i++)
                                            if (DisplayCount > 0)
                                            {
                                                serialPortDisplay.Write(header);
                                                //m_socWorker[i].Send(byData);
                                                Thread.Sleep(50);
                                            }
                                ...
                                if (int.Parse(reader["alarmid"].ToString()) < 1000 && reader["DisplayText"].ToString() != previousalarmtext && reader["AlarmCount"].ToString() != "1")
                                {
                                    message = " ";
                                    header = "";
                                    header = header + "^A000000^E0^C1";
                                    header = header + message + (char)13;
                                    sentblank = true;
                                    objData = header;
                                    byData = System.Text.Encoding.ASCII.GetBytes(objData.ToString());
                                    if (dontbother == false)
